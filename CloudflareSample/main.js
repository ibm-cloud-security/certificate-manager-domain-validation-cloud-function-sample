 ...
 */
const addTxtRecord = async (zoneId, domain, payload, userInfo) => {
    ...
    const options = {
        ...
        json: {
            ...
            content: recordValue,      //The TXT record value.
            ttl: 120,
        }
    };
    let response;
    ...
    if (response.statusCode === 200 && response.body.success) {
        ...
        response.body.errors[0].message === 'The record already exists.') {
        console.log(`TXT record "${recordValue}" already exists in zone ${zoneId}`);
    }
    else {
...
const getTxtRecords = async (zoneId, domain, payload, userInfo) => {
    ...
    console.log(`Get records named ${recordName} returned ${response.body.result.length} results.`);
    return response.body.result;
};

/**
 * Delete a single TXT record from zone.
 * @param zoneId
 * @param recordId
 * @param userInfo user credentials
 * @returns {Promise<void>}
 */
const removeTxtRecord = async (zoneId, recordId, userInfo) => {
...
const setChallenge = async (payload, userInfo) => {
    ...
    await addTxtRecord(zoneId, domain, payload, userInfo);
};
